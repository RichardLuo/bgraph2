...
import bgraph
import bgraph.utils
from bgraph.types import List, Dict, Tuple, Optional, Union, Section, depends_on, getSectionKey
...
def combine_files_path(branch_dir: Path) -> Dict[Path, List[str]]:
    ...
    for file_path in branch_dir.rglob("files.pickle"):
        try:
            ...
            logger.info("pickle load %s ok", file_path)
        except pickle.PickleError:
            logger.info("pickle load %s error!!!", file_path)
            continue

        files[file_path.parent] = local_files

    for key, value in files.items():
        logg.info("files[%s] = %s", key, value)
...
def test_build_dep_map(section : Section, sections: List[Section]) -> list:
    edges : list = []
    for sec in sections:
        if sec == section:
            continue
        elif depends_on(sec, section):
            logger.info("up section  %s <-- down %s", getSectionKey(sec), getSectionKey(section))
            edge = (getSectionKey(sec), getSectionKey(section))
            edges.append(edge)
        elif depends_on(section, sec):
            edge = (getSectionKey(section), getSectionKey(sec))
            edges.append(edge)
            logger.info("up section  %s <-- down %s", getSectionKey(section), getSectionKey(sec))
    print("== edges:", edges);
    return edges
            
@bgraph.utils.no_except
def compose_local_manifest_branch(
        aosp_root: Union[str, Path],
        custom_name : str,
        work_dir: Optional[Path] = None,
        force: bool = False,
) -> Optional[Path]:
    ...
    :param aosp_root: Path/Link towards the aosp_root or the manifest URL
    :param work_dir: Optional. Working directory - if not set, a temporary folder is used
    :param force: Optional. Overwrite existing branch.
    :return: The path to the work dir
    """
    manifest_file = aosp_root / ".repo" / "manifests" / "default.xml"
    ...
    branch_name = manifest.get_revision()

    # Guard: do not redo a branch
    pickle_file = work_dir / f"{branch_name}-{custom_name}.pickle"

    logger.info("Start local composing for %s %s, pick file %s", manifest_file, branch_name, pickle_file)
    ...
    branch_dir = work_dir / branch_name
    branch_dir.mkdir(exist_ok=True)

    logger.info("Starting parsing AOSP build files, revision:%s", manifest.get_revision())

    soong_parser = bgraph.parsers.SoongParser()
    soong_parser.parse_aosp(aosp_root, project_map=manifest.get_projects())
    soong_parser.file_listing = combine_files_path(branch_dir)

    # for section in soong_parser.section_list:
    #     print("\n")
    #     for key in section:
    #         print(key + ":" + repr(section[key]))

    # for sec in soong_parser.section_list:
    #     if (sec['section_name'] is not None):
    #         logger.info(">> add_node %s", getSectionKey(sec))
    # for sec in soong_parser.section_list:
    #     if (sec['section_name'] is not None):
    #         logger.info(">> test_build_source_map %s", getSectionKey(sec))
    #         test_build_dep_map(sec, soong_parser.section_list)

    # logger.info(">> dump_section_types")
    # soong_parser.dump_section_types()
    # logger.info("<< dump_section_types")

    # Save the result
    try:
        logger.info(">> dump pick file %s", pickle_file)
        pickle.dump(soong_parser, open(pickle_file, "wb"))
    except pickle.PickleError:
        logger.error("Failed to pickle")
        # clean_disk(branch_dir)
