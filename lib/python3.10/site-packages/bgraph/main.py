...
from bgraph.parsers.soong_parser import SoongParser
from bgraph.backup import get_aosp_root, test_subdirs, backup_module, gen_backup_path, restore_module, is_normal_module_path, is_backup_module_path, conv_to_normal_path, conv_to_backup_path, restore_from_backup_root, validate_module_path
...
def check_if_in_modules(path : Path, module_paths : Set[Path]) -> bool:
    for m in module_paths:
        if m == path or m.parent in path.parents:
            return True
    return False
...
class GraphShell(cmd2.Cmd):
    ...
    dump_parser.add_argument('-B', '--backup_modules_file', type=str, default="backup-modules.conf", help='dump android.bp path into module files')
    ...
    def compile_aosp(self,
        ...
        with subprocess.Popen(command,
            ...
            if success is True:
                logger.info("== COMPILED OK FOR MODULE %s ==", str(module_path))
            else:
                if fixed_comment:
                    self.write_line_to(fixed_modules_file, 'a', fixed_comment)
                    self.write_line_to(fixed_modules_file, 'a', str(module_path))
    ...
    def do_auto_backup(self, args : argparse.Namespace):
        fixed_modules = set(load_modules_file(args.fixed_modules_file))
        with open(args.modules_file, 'r', encoding='utf-8') as modules_file:
            ...
            for path in paths:
                if not path.exists():
                    ...
                    success, set_fixed = self.compile_aosp(path, args.fixed_modules_file)
                    if success:
                        ...
                        restore_module(backup_path, args.dry_run)
                        if set_fixed is True:
                            fixed_modules.add(path)
                            logger.info(f"{path} WAS SET FIXED, KEEP GOING")
    ...
    def do_restore(self, args : argparse.Namespace):
        if args.single_module:
            ...
            if is_backup_module_path(path):
                ...
                logger.info("restore single module path %s dry_run:%d", path, args.dry_run)
                if success:
    ...
    def do_restore_all_backup(self, args : argparse.Namespace):
        ...
        with open(args.backup_modules_file, 'w', encoding='utf-8') as file:
            for path in module_paths:
                file.write("%s\n" % str(path))
