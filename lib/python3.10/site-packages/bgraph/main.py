...
class SoongFile:
    ...
    def is_independent_module(self) -> bool:
        soong_files = self.up_soong_files.union(self.child_soong_files)
        for sf in soong_files:
            if not self.path.parent in list(sf.path.parents):
                return False
        return True
...
class GraphShell(cmd2.Cmd):
    ...
    def setup_dependency_by_names(self,
        ...
        for name in names:
            ...
            if name in self.DEBUG_SECTION_NAMES:
                logger.info("DEP_WAY:%s names:%s", dep_way, repr(names))
            for down in down_nodes:
                if name in self.DEBUG_SECTION_NAMES:
                    logger.info("dep_way:%s name:%s, down-node:%s", dep_way, name, down.name2())
                if (dep_way == 'certificate' and
                    down.section['section_type'] != 'android_app_certificate'):
                    continue
                elif check_section_name_matching(name, down.section):
                    if check_node_matching(up, down) or len(down_nodes) == 1:
                        setup_up_down_dependency(up, down, dep_way, edges)
                        self.setup_project_and_soong_files_dep(down)
                    else:
                        pass
    ...
    def setup_project_and_soong_files_dep(self, down_node : SectionNode):
        ...
        for up_node in down_node.up_level_nodes:
            ...
            self.expand_soong_child(down_soong_file)
    ...
    def build_section_graph(self,
        ...
        if max_levels <= 0:
            ...
            # self.generate_edges_from_down(down_node, dep_type, edges)
            self.generate_edges_from_up(down_node, edges)
    ...
    def do_gen_nodes_graph(self, args : argparse.Namespace):
        ...
        for n in total_nodes:
            ...
            self.build_section_graph(self.UNLIMITED_LEVELS, n, DependType.UP, edges)
    ...
    def dump_file_graph_with_level(self,
                                   levels : int,
                                   max_levels : int,
        ...
        if levels < max_levels:
            if not undumped_files:
                ...
                for file in undumped_files:
                    logger.info("handle soong %s, up_soong_files:%d dumped_files:%d", file.path, len(file.up_soong_files), len(dumped_files))
                    valid_up_files = file.up_soong_files.difference(dumped_files)
                ...
                self.dump_file_graph_with_level(levels + 1, max_levels, undumped_files, dumped_files, modules_file)
        else:
    ...
    def do_dump(self, args : argparse.Namespace):
        ...
        if args.type == self.ALL_MODULE_TYPES:
            ...
            with open(args.modules_file, 'w', encoding='utf-8') as modules_file:
                ...
                self.dump_file_graph_with_level(0, args.max_levels, total_files, set(), modules_file)
