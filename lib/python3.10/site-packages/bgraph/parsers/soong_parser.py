...
import bgraph.exc
import bgraph.utils
from bgraph.types import (Any, Set, Dict, Final, List, Literal, Optional, Section, SectionNode,
                          TypedDict, Union, cast, overload, getSectionKey)

from bgraph.backup import deep_dump_dict
...
class Manifest:
    ...
    def get_revision(self) -> str:
        """Returns the revision of manifest.
        :return: revision
        """
        return os.path.basename(self.xml.manifest.default["revision"])
...
class ParserSet(TypedDict, total=False):
    soong_parser: Any
...
class SoongParser:
    ...
    SECTION_TYPE: Final = "section_type"
    ...
    SOONG_FILE: Final = "soong_file_path"
    ...
    def __init__(self) -> None:
        """Init method."""
        # self.parsers: ParserSet = collections.defaultdict(list)
        self.variables: Dict[str, Any] = {}
        self.section_nodes: List[SectionNode] = []
        self._files_listing: Dict[pathlib.Path, List[str]] = {}
        self.sections: Dict[str, Section] = collections.defaultdict(list)
        self.section_types = set()
        self.empty_soong_file_paths : Set[pathlib.Path] = set()
    ...
    def list_section(self, with_defaults: bool = False) -> List[str]:
        ...
        for section_key, target in self.sections.items():
            if with_defaults or "default" not in target.get(self.SECTION_TYPE, ""):
    ...
    def dump_section(self, info : str, section : Section) -> None:
        logger.info("%s section %s, type:%s soong-bp:%s",
                    info, getSectionKey(section), section[self.SECTION_TYPE], section[self.SOONG_FILE])

    def dump_section_types(self) -> None:
        for t in self.section_types:
            logger.info("-- section_type:%s", t)
    ...
    def parse_file(
        ...
        # logger.info("-> parse file:%s path:%s name:%s", file_path, project_path, project_name)
        try:
        ...
        if parser.is_empty():
            self.empty_soong_file_paths.add(parser.file_path)
        ...
        if project_name is None:
            logger.error("None project name!")
            return None
        else:
            soong_file_path = pathlib.Path(file_path)
            for section_name, sections in parser.sections.items():
                for section in sections:
                    ...
                    section["soong_file_path"] = soong_file_path
                    self.section_types.add(section[self.SECTION_TYPE])
                    # self.dump_section(section)
                    # Self.sections[section_name].append(section)
                    section_key = getSectionKey(section)
                    if section_key in self.sections:
                        logger.error("section %s already exist!", section_key)
                        self.dump_section("new section", section)
                        self.dump_section("old section", self.sections[section_key])
                        raise bgraph.exc.BGraphConflictSection("section conflicts!")
                    else:
                        self.sections[section_key] = section
                        self.section_nodes.append(SectionNode(section))

            build_files : List[str] = []
            for key, values in project_variables.items():
                if key == 'build':
                    if isinstance(values, str):
                        build_files.append(values)
                    elif isinstance(values, list):
                        build_files.extend(values)
                        
                    logger.info("append build files %s type:%s from %s", repr(values), type(values), file_path)

            project_variables.update(parser.variables)
            self.variables.update(parser.variables)

            return [ soong_file_path.parents[0] / f for f in build_files ]
    ...
    def parse_aosp(
        ...
        for project_name, relative_path in project_map.items():
            ...
            logger.info(">> parse project:%s path:%s", project_name, project_path)
            self.parse_project(
    ...
    def parse_project(
        ...
        file_names = ('Android.bp', 'Android.source.bp')
        soong_file_list = []
        for name in file_names:
            soong_file_list.extend(project_directory.rglob(name))

        for soong_file in soong_file_list:
            if project_directory == pathlib.Path("external/nos/host/generic"):
                logger.info(">> parse file [%s] of project [%s]", soong_file, project_directory)

            build_files = self.parse_file(
                file_path=soong_file,
                project_name=project_name,
                project_path=project_directory,
                project_variables=project_variables)
            if build_files:
                logger.info("build files:%s", repr(build_files))
    ...
    def get_targets(self) -> List[str]:
        ...
        for section_name in self.list_section(with_defaults=False):
            ...
            for section in section_map:
                section_type = section.get(SoongParser.SECTION_TYPE)
    ...
    def _retrieve_section(
        self, section_key: str, recursive: bool = False
    ) -> Union[List[Section], Section]:
        ...
        for index, section_map in enumerate(projects):
            ...
            logger.error("HOW TO RUN HERE!")
            # self.sections[section_key][index] = section_map
            del self.sections[section_key][index]["defaults"]
            raise bgraph.exc.BGraphMissingSectionException

        if recursive is True:
            return self.sections[section_key][0]
        else:
...
class SoongFileParser:
    ...
    def __init__(
        ...
        if self.file_path.exists():
            ...
            if self.identifiers:
                logger.error("%s has identifiers:%s", self.file_path, repr(list(self.identifiers)))
                raise bgraph.exc.BGraphParserException("Identifiers error ocured!")
            elif not self.sections:
                logger.error("%s has no sessions!, remain it!", self.file_path)
                # raise bgraph.exc.BGraphParserException("An error of no session!")
            elif not self.variables:
                pass
                # logger.error("%s has no variables!", self.file_path)
                # raise bgraph.exc.BGraphParserException("An error of no variables!")

    def is_empty(self) -> bool:
        return True if len(self.sections) == 0 and self.file_path.exists() else False
    ...
    def parse_variable_ref(self, tokens: List[str]) -> Any:
        ...
        if var_name not in self.variables:
            logger.error("var_name %s not in here %d", var_name, len(self.variables))
            self.variables[var_name] = 'UNSUPPORTED_VARIABLE_NEED_FIXED_LATTER'
    ...
    def parse_variable_def(
        ...
        if append is False:
            ...
            # logger.info("new one: variable_name:%s -> new_value:%s", variable_name, new_value)
            ...
            # logger.info("append one: variable_name:%s -> new_value:%s", variable_name, actual_value + new_value)
            self.variables[variable_name] = actual_value + new_value

    @staticmethod
    def parse_dict_list(tokens: List[str]) -> Optional[str]:
        """Helper method to concat string together

        :param tokens: Tokens
        :return: Optionnaly a string string
        """
        """--> parse_dict_list: TOKENS:1,([{'version': '1', 'imports': ['android.hardware.common-V2', 'android.hardware.common.fmq-V1']}], {}) TOKENS:2#(['versions_with_info', {'version': '1', 'imports': ['android.hardware.common-V2', 'android.hardware.common.fmq-V1']}], {})# identifiers:['name', 'vendor_available', 'srcs', 'imports', 'stability', 'backend', 'versions_with_info']"""

        # logger.info("--> parse_dict_list: TOKENS:%d,%s", len(tokens), repr(tokens))

        result : List[Dict] = []
        for token in tokens:
            if isinstance(token, dict):
                result.append(token)
                # logger.info("append token %s", repr(token))
                # deep_dump_dict(token, 1, "dump the token")
    ...
    def parse_list_concat(tokens: List[str]) -> List[Any]:
        ...
        """
        # logger.error("--> parse_list_concat: TOKENS:%d,%s", len(tokens), repr(tokens))
    ...
    def parse_section_field(self, tokens: List[str]) -> str:
        ...
        # logger.error("TOKENS:%d#%s# identifiers:%s", len(tokens), repr(tokens), repr(list(self.identifiers)))
    ...
    def parse_section(self, tokens: List[str]) -> None:
        ...
        # section_dict.section_type = tokens[0]
        for token in tokens[1:]:
            if token in self.identifiers:
                ...
                # logger.error("after del token %s, identifiers:%s", token, repr(list(self.identifiers)))
            else:
                raise bgraph.exc.BGraphParserException(
        ...
        if section_name is None:
            ...
            if section_dict[SoongParser.SECTION_TYPE] == "soong_namespace":
                logger.debug("Found a soong namespace but it is not supported yet.")
            elif section_dict[SoongParser.SECTION_TYPE] == "package":
                pass            # ignore
            else:
                logger.debug("Found a section without name! type:%s", section_dict[SoongParser.SECTION_TYPE])
    ...
    def _init_parser(self) -> pyparsing.ParserElement:
        ...
        dict_list = (
            lbrack
            + pyparsing.ZeroOrMore(pyparsing.delimitedList(dict_def, delim=comma))
            + pyparsing.Optional(comma)
            + rbrack
        )
        dict_list.setParseAction(self.parse_dict_list).setName("dict-list")
